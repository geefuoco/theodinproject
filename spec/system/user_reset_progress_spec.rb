...
RSpec.describe 'User Reset Progress', type: :system do
  ...
  let!(:first_completion) { create(:lesson_completion, lesson: foundation_first_lesson, user: user) }
  let!(:second_completion) { create(:lesson_completion, lesson: rails_first_lesson, user: user) }

  # REFACTOR: create lesson completion records before the test is run
